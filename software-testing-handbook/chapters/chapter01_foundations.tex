...
\subsection{Risk Scoring Rubric}
...
\begin{lstlisting}[language=python, caption={Assertion helper that keeps expectations explicit.}, label=lst:ch01-assertion-helper]
risk_scores = []
...
\textbf{How to run}
\begin{itemize}
  \item \texttt{python examples/ch01/risk\_score.py}
  \item \texttt{python examples/ch01/deterministic\_assertions.py}
\end{itemize}
...
\section{Summary}
- The risk scoring rubric gives deterministic gate outputs that feed the pyramid sizing heuristics in Chapter 2.
- Deterministic assertions and explicit coverage narratives anchor regressions so downstream chapters can layer automation on stable foundations.

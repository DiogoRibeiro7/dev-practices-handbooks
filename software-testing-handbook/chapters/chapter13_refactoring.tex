...
The worked example in \texttt{examples/ch13/report\_handler.py} shows a handler that originally creates \texttt{DatabaseClient}, \texttt{EmailNotifier}, and \texttt{Clock} instances internally. The refactored version accepts those collaborators via constructor injection and uses fakes for tests. Run \texttt{examples/ch13/README.md} to see how the deterministic harness replaces real integrations in CI while the production wiring can still instantiate the real clients.

\begin{lstlisting}[language=python, caption={Refactoring checklist automation.}, label=lst:ch13-refactor-checklist]
from examples.ch13.report_handler import (
    Clock,
    DatabaseClient,
    EmailNotifier,
    ReportHandler,
...
handler.run({"id": "order-42", "owner": "ops"})
\end{lstlisting}

The snippet uses \texttt{build\_default\_handler()} to construct the production wiring (real DB client, notifier, clock). Tests call `ReportHandler` directly with fakes to keep suites fast and deterministic.

...
\subsubsection{The Solution: 2$	imes$2 Factorial Design}
...
\bottomrule
\end{tabular}
\caption{2$	imes$2 factorial design with all treatment combinations}
\end{table}
...
\subsubsection{2$	imes$2 Factorial Model}
...
\begin{itemize}
    \item Model A: 200K users $\times$ 3.2\% conversion $\times$ \$120/year = \$768K
    \item Model B: 200K users $\times$ 4.1\% conversion $\times$ \$120/year = \$984K
...
\begin{itemize}
    ...
    \item Annual churn increase: -3pp $\times$ 200K = 6,000 users/year
    \item Average LTV per user: \$180 (3 years $\times$ \$60/year average)
    \item \textbf{Loss}: 6,000 users $\times$ \$180 = \$1.08M over 3 years
\end{itemize}
...
\begin{itemize}
    \item Conversion: 3.2\% $\to$ 3.7\% (+15.6\% blended)
    \item Listening time: 65 min $\to$ 63 min (-3\% blended)
    \item Retention: 82\% $\to$ 81\% (-1.2\% blended)
    \item Revenue: \$8.20 $\to$ \$9.00 (+9.8\% blended)
...
if decision['overall_decision'] == 'DEPLOY':
    print("\n[OK] Deploying variant based on results")
    # Deploy via feature flag system
elif decision['overall_decision'] == 'DO_NOT_DEPLOY':
    print("\n[X] Not deploying - results do not support launch")
    # Keep current experience
else:
    print("\n[PAUSE] Inconclusive - continuing test")
...
\begin{itemize}
    ...
    \item \textbf{Annual Value}: $23\% \times \$8\text{B GMV} = \$1.84\text{B in incremental GMV}$
    \item \textbf{ROI}: Recommendation system costs \$12M/year $\to$ \textbf{154x ROI}
...
class SyntheticControl:
    ...
    def fit(
        ...
        donor_pre_matrix = np.array(donor_pre_matrix).T  # Time $	imes$ Donors
    ...
    def _optimize_weights(
        ...
        Args:
            ...
            X_donors: Characteristics matrix of donors (Features $	imes$ Donors)
...
class DifferenceInDifferences:
    ...
    def _test_parallel_trends(
        ...
        Regresses outcome on treatment $	imes$ time interaction in pre-period.
        ...
        # Test treatment $	imes$ time interaction
...
class RegressionDiscontinuity:
    ...
    def _bandwidth_sensitivity(
        ...
        # Test range: 0.5 $	imes$ optimal to 2 $	imes$ optimal
...
class ExperimentDurationPlanner:
    ...
    def plan_duration(
        ...
        if seasonal_risk == 'low':
            recommendations.append(
                f"[OK] No major seasonality conflicts. Proceed with {start_date.strftime('%Y-%m-%d')} start."
            )
